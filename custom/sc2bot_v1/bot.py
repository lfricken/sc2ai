...
class RampWallBot(sc2.BotAI):
	...
	async def on_step(self, iteration):
		...
		if self.can_afford(Unit.COMMANDCENTER):
		...
		for depo in self.units(Unit.SUPPLYDEPOT).ready:
			for unit in self.known_enemy_units.not_structure:
				if unit.position.to2.distance_to(depo.position.to2) < 15:
					break
			else:
				await self.do(depo(Unit.MORPH_SUPPLYDEPOT_LOWER))

		# Lower depos when no enemies are nearby
		for depo in self.units(Unit.SUPPLYDEPOTLOWERED).ready:
			for unit in self.known_enemy_units.not_structure:
				if unit.position.to2.distance_to(depo.position.to2) < 10:
					await self.do(depo(Unit.MORPH_SUPPLYDEPOT_RAISE))
					break
